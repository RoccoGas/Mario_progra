...
enum ERROR_OPTIONS_E start_menu() {
    ...
    const char* bitmapBackgroundFilepath = "assets//menu/menu_background.png";
    const char* bitmapOceanFilepath = "assets/menu/8bit_ocean.jfif";
    const char* fontFilepath = "assets/menu/mario_font.ttf";
    ...
    bitmapOcean = al_load_bitmap(bitmapOceanFilepath);
    if (bitmapOcean == NULL) {
        fprintf(stderr, "Failed to load %s\n", bitmapOceanFilepath);
        return BAD_ASSET;
    }
    ...
    while (menuLoop) {
        ...
        case ALLEGRO_EVENT_TIMER:
            al_draw_bitmap(bitmapOcean, 0, 0, NO_FLAGS); 
...
enum ERROR_OPTIONS_E pause_menu(ALLEGRO_DISPLAY* display) { //igual q menu de comienzo solo, que no genera un nuevo display, usa el viejo
    ...
    if (display == NULL) {
        fprintf(stderr, "Failed to recive menu display \n ");
        return BAD_DISPLAY;
    }
